...
4. End with either:
   - a ready-to-normalize interview result for `rupify-discovery`, or
...
- Hand off to `rupify-discovery` once the interview is complete enough to normalize.

## Routing

- Use `rupify-discovery` after the interview to update `rupify-model.yaml`.
- Use `rupify` only when the user wants the full interview-to-artifact workflow in one pass.
